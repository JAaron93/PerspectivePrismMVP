...
function validateCacheDuration() {
    ...
    if (isNaN(duration) || duration < 1 || duration > 168) {
        cacheDurationError.textContent = 'Cache duration must be between 1 and 168 hours';
...
async function testConnection() {
    ...
    try {
        ...
        if (error.name === 'AbortError') {
            showBackendUrlError('Connection timeout: Server did not respond within 10 seconds');
        } else if (error.message.includes('fetch')) {
            showBackendUrlError('Cannot connect to server. Check the URL and try again.');
        } else {
...
async function clearAllData() {
    if (!confirm('Are you sure you want to clear all cached analysis data? This cannot be undone.')) {
        return;
    }
    ...
    try {
        ...
        const cacheKeys = Object.keys(allKeys).filter(key =>
            key.startsWith('cache_') || key === 'cache_metadata'
        );
...
function viewPrivacyPolicy() {
    ...
    // In production, this should point to the actual privacy policy
    chrome.tabs.create({
        url: 'https://github.com/yourusername/perspective-prism/blob/main/PRIVACY.md'
    });
}
